# 탈출
# visited를 방문 겸 이동 거리를 같이 포함해서 사용하였고
# 물과 시작지점 이외의 위치들은 if조건을 통해 반례가 없도록 하였습니다.
...
# BFS 함수 선언
def BFS():
    q_biber = deque()                                                           # 물과 비버를 deque로 선언하고
    q_water = deque()
    q_biber.append(biber_pos)                                                   # 스타트 지점을 append 해주고
    for i in range(len(water_pos)):                                             # 처음에 물이 몇개인지 모르기에 찾은 물의 위치만큼 append
        q_water.append(water_pos[i])

    while True:                                                                 # 반복문
        if not q_biber:                                                         # 비버가 더이상 움직일 수 없다면 while문 종료
            break

        for _ in range(len(q_water)):                                           # q_water에 담겨있는 값들을 꺼내서 범위 안에 있고 탐색 범위가 .이라면
            iw, jw = q_water.popleft()                                          # matrix의 . => * 바꿈
            for k in range(4):
                niw = iw + di[k]
                njw = jw + dj[k]
